...
To test this functionality, we can intentionally cause an error by attempting to listen on a port that is already in use. For example, we can crate a new file called **otherServer.js** using the code from [introduction]("cerebro-stream.com/Subjects/BackEnd/NodeJS/Introduction") and then run them simultanously.

![Error Image](/Articles/BackEnd/NodeJS/error2.png)
![Error Image](/Articles/BackEnd/NodeJS/error3.png)
...
When we run both servers, we should see an error message in the console indicating that port 3001 is already in use.

![Error Image](/Articles/BackEnd/NodeJS/error1.png)
